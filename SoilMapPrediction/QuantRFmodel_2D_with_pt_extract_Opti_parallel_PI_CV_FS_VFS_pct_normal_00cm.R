...
shp.pts <- ptsc[,-c(9,10)]
shp.pts$sand_f_vf_psa<-shp.pts$sand_f_psa+shp.pts$sand_vf_psa
...
## Save points
setwd("O:/Models_active_work/UpCo/FS_VFS_pct_2D_CV")
write.table(pts.ext, "cop_ncss17_FS_VFS_pct_covarsc.txt", sep = "\t", row.names = FALSE)
...
pts.extc <- subset(pts.ext, as.numeric(pts.ext$hzn_top) <= 100 & as.numeric(pts.ext$hzn_bot) > 100) # subset to chosen depth
ptspred.list <- gsub(".tif","", cov.grids)# Take .tif off of the grid list to just get the variable names
pred <- "sand_f_vf_psa" ## Dependent variable
...
pts.extc  <- subset(pts.extc, pts.extc$sand_f_vf_psa!= "NA")
...
trainy = pts.extc[c("sand_f_vf_psa")]
set.seed(41)
## rfcv runs a cross val and tests variable importance
nfolds <- 10
cv.rf = rfcv(trainx, (trainy$sand_f_vf_psa), cv.fold = nfolds, proximity=FALSE, ntree=100, keep.forest=TRUE)
...
cv.RMSE = sqrt(mean(((pts.extc$sand_f_vf_psa) - pts.extc$cvpred)^2, na.rm=TRUE))
cv.Rsquared = 1-var((pts.extc$sand_f_vf_psa) - pts.extc$cvpred, na.rm=TRUE)/var((pts.extc$sand_f_vf_psa), na.rm=TRUE)
NS<-NSE(pts.extc$cvpred, (pts.extc$sand_f_vf_psa))
