...
nfolds <- 10
cv.rf = rfcv(trainx, sqrt(trainy$sand_f_vf_psa), cv.fold = nfolds, proximity=FALSE, ntree=100, keep.forest=TRUE)
...
cv.RMSE = sqrt(mean((sqrt(pts.extc$sand_f_vf_psa) - pts.extc$cvpred)^2, na.rm=TRUE))
cv.Rsquared = 1-var(sqrt(pts.extc$sand_f_vf_psa) - pts.extc$cvpred, na.rm=TRUE)/var(sqrt(pts.extc$sand_f_vf_psa), na.rm=TRUE)
NS<-NSE(pts.extc$cvpred, sqrt(pts.extc$sand_f_vf_psa))
